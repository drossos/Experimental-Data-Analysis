...
x_dat = csvread('bb_data/x_dat.csv');
y_dat = csvread('bb_data/y_dat.csv');
y_uncert = csvread('bb_data/y_uncert.csv');
x_uncert = csvread('bb_data/x_uncert.csv');
%y_uncert = zeros(size(x_dat));
%x_uncert = zeros(size(x_dat));
...
y_uncerts = zeros(size(y)) + y_uncert;
x_uncerts = zeros(size(x)) + x_uncert;
...
str = [sprintf("m = %g Area/T^4 +- %g Area/T^4",round(m,digit,'significant'),round(sm,digit,'significant')), sprintf("b = %g Area +- %g Area",round(b,digit,'significant'),round(sb,digit,'significant')), sprintf("r^2 = %g",round(r2,digit,'significant')),  sprintf('\\chi^2 = %g',round(chi_sqr,digit,'significant')) ,  sprintf('\\chi^2_v = %g',round(red_chi_sqr,digit,'significant'))]

annotation('textbox',[.15 .55 .35 .35],'String',str,'FitBoxToText','on');

%Saves plot in curr direct
saveas(gcf, "bb_data/Linear Regression.png")

%plot residual graph
residuals(x,y,lin_regress)

saveas(gcf, "bb_data/Residuals Plot.png")
